...
const locales = {
  ...
  es: {
    days: ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sabádo"],
  ...
  <div class="datepicker-picker inline-block rounded-lg bg-white shadow-lg p-4">
    <div class="datepicker-header">
      ...
      <div class="datepicker-controls flex justify-between mb-2">
        <button type="button" class="bg-white rounded-lg text-gray-500 hover:text-gray-900 text-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-gray-200 prev-btn"></button>
        <button type="button" class="text-md rounded-lg text-gray-900 bg-white font-semibold py-2.5 px-5 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 view-switch"></button>
        <button type="button" class="bg-white rounded-lg text-gray-500 hover:text-gray-900 text-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-gray-200 next-btn"></button>
      </div>
    </div>
    <div class="datepicker-main p-1"></div>
    <div class="datepicker-footer">
      <div class="datepicker-controls flex space-x-2 mt-2">
        <button type="button" class="%buttonClass% today-btn text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-md px-5 py-2 text-center w-1/2"></button>
        <button type="button" class="%buttonClass% clear-btn text-gray-900 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-md px-5 py-2 text-center w-1/2"></button>
      </div>
    </div>
  </div>
</div>`);
...
const daysTemplate = optimizeTemplateHTML(`<div class="days">
  ...
  <div class="datepicker-grid w-64 grid grid-cols-7">${createTagRepeat('span', 42 , {class: 'block flex-1 leading-9 border-0 rounded-lg cursor-default text-center text-gray-900 font-semibold text-md h-6 leading-6 text-md font-medium text-gray-500'})}</div>
</div>`);
...
const calendarWeeksTemplate = optimizeTemplateHTML(`<div class="calendar-weeks">
  <div class="days-of-week flex"><span class="dow h-6 leading-6 text-md font-medium text-gray-500"></span></div>
...
class DaysView extends View {
  ...
  setOptions(options) {
    ...
    if (updateDOW) {
      Array.from(this.dow.children).forEach((el, index) => {
        ...
        el.className = this.daysOfWeekDisabled.includes(dow) ? 'dow disabled text-center h-6 leading-6 text-md font-medium text-gray-500 cursor-not-allowed' : 'dow text-center h-6 leading-6 text-md font-medium text-gray-500';
      });
    }
  }
  ...
  render() {
    ...
    Array.from(this.grid.children).forEach((el, index) => {
      ...
      el.className = `datepicker-cell hover:bg-gray-100 block flex-1 leading-9 border-0 rounded-lg cursor-pointer text-center text-gray-900 font-semibold text-md ${this.cellClass}`;
      el.dataset.date = current;
      el.textContent = date.getDate();

      if (current < this.first) {
        classList.add('prev', 'text-gray-500', 'dark:text-gray-500');
      } else if (current > this.last) {
        classList.add('next', 'text-gray-500', 'dark:text-gray-500');
      }
      if (this.today === current) {
        classList.add('today', 'bg-gray-100', 'bg-gray-100', 'bg-gray-100');
      }
      ...
      if (this.range) {
        ...
        if (current > rangeStart && current < rangeEnd) {
          classList.add('range', 'bg-gray-200', 'bg-gray-100');
          classList.remove('rounded-lg', 'rounded-l-lg', 'rounded-r-lg')
        }
        if (current === rangeStart) {
          classList.add('range-start', 'bg-gray-100', 'bg-gray-100', 'rounded-l-lg');
          classList.remove('rounded-lg', 'rounded-r-lg');
        }
        if (current === rangeEnd) {
          classList.add('range-end', 'bg-gray-100', 'bg-gray-100', 'rounded-r-lg');
          classList.remove('rounded-lg', 'rounded-l-lg');
        }
      }
      if (this.selected.includes(current)) {
        classList.add('selected', 'bg-blue-700', 'text-white', 'dark:bg-purple-darken', 'dark:text-gray-500');
        classList.remove('text-gray-900', 'text-gray-500', 'hover:bg-gray-100', 'text-gray-500', 'dark:hover:bg-gray-600');
      }
  ...
  refresh() {
    ...
    this.grid
      ...
      .forEach((el) => {
        el.classList.remove('range', 'range-start', 'range-end', 'selected', 'bg-blue-700', 'text-white', 'dark:bg-purple-darken', 'dark:text-gray-500', 'focused', 'bg-gray-100', 'bg-gray-100');
        el.classList.add('text-gray-900', 'rounded-lg', 'text-gray-900');
      });
    Array.from(this.grid.children).forEach((el) => {
      ...
      if (current === rangeStart) {
        classList.add('range-start', 'bg-gray-200', 'bg-gray-100', 'rounded-l-lg');
        classList.remove('rounded-lg', 'rounded-r-lg');
      }
      if (current === rangeEnd) {
        classList.add('range-end', 'bg-gray-200', 'bg-gray-100', 'rounded-r-lg');
        classList.remove('rounded-lg', 'rounded-l-lg');
      }
      if (this.selected.includes(current)) {
        classList.add('selected', 'bg-blue-700', 'text-white', 'dark:bg-purple-darken', 'dark:text-gray-500');
        classList.remove('text-gray-900', 'hover:bg-gray-100', 'dark:text-gray-500', 'dark:hover:bg-gray-600');
      }
      if (current === this.focused) {
        classList.add('focused', 'bg-gray-100', 'bg-gray-100');
      }
    });
  }
  ...
  refreshFocus() {
    ...
    this.grid.querySelectorAll('.focused').forEach((el) => {
      el.classList.remove('focused', 'bg-gray-100', 'bg-gray-100');
    });
    this.grid.children[index].classList.add('focused', 'bg-gray-100', 'bg-gray-100');
  }
}
...
class MonthsView extends View {
  ...
  refresh() {
    ...
    this.grid
      .querySelectorAll('.range, .range-start, .range-end, .selected, .focused')
      .forEach((el) => {
        el.classList.remove('range', 'range-start', 'range-end', 'selected', 'bg-purple-darken', 'bg-purple-darken', 'dark:text-gray-500', 'text-white', 'focused', 'bg-gray-100', 'bg-gray-100');
        el.classList.add('text-gray-900', 'hover:bg-gray-100', 'dark:text-gray-500', 'dark:hover:bg-gray-600');
      });
    Array.from(this.grid.children).forEach((el, index) => {
      const classList = el.classList;
      if (index > rangeStart && index < rangeEnd) {
  ...
  refreshFocus() {
    ...
    this.grid.children[this.focused].classList.add('focused', 'bg-gray-100', 'bg-gray-100');
  }
}
